...
function search_go(num) {
	...
	$.ajax ({
	    ...
	    success: function (data) {
	    	...
	    	$(data.tbsView).each(function(i,e){
	    		...
	    		$('tbody').append('<tr class="row ajaxTr">' +
			    						   ...
			    						   '<td>' + e.cpn_ADDR + '</td>' +
			    						   '<td>' + e.cpn_STATE + '</td>' +
			    						   '<td nowrap>' + phoneNumberFormat(e.cpn_TEL) + '</td>' +
			    						'</tr>'
	    		);
	    	});
	    	if(prev){
...
<section class='div_header'>
    <div class='search_div' onkeyup='onEnterSearch()'>
        <select class='select_box' id = "type">
            <option value='snm'>매장명</option>
            <option value='scd'>매장코드</option>
        </select>
...
<div class='main_div'>
    <table>
        <thead>
	        <tr>
	            <th colspan='7' class="mtitle">매장 리스트</th>
	        </tr>
	        <tr id="searchTr">
		        <th>순번</th>
	            <th>매장코드</th>
