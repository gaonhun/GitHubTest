...
<html>
<head>
<title>배달 대행사 관리</title>
</head>
<script src="./resources/js/jquery-3.6.0.min.js"></script>
<link rel="stylesheet" type="text/css" href="./resources/css/oms_common.css" />
<link rel="stylesheet" type="text/css" href="./resources/css/oms_sub05_03.css" />
...
function search_go(num) {
	...
	const cpncd = $('#dccd').val();
	const sst = $('#sst').val();
	const pageNum = num;

	$.ajax({
		url: 'oms_sub05_03_process',
		dataType: 'json',
		type: 'POST',
		async: false,
		data: {
		...
		success: function(data) {
			...
			let obLength = Object.keys(data.dvlList).length;
			...
			if (obLength == 0) {
				...
				$(data.dvlList).each(function(i, e) {
...
$(document).on("dblclick", "td", function() {
	...
	form.target = "_blank";
	form.method = "POST";
	form.action = "oms_sub05_03_detail";
...
<body>
	...
	<div class="page_div">
		<header>
			<div id="header_title"><p>시스템관리 > 배달대행사관리</p></div>
		...
		<section class='div_header'>
			...
			<div class="btn_div">
				<button type="button" class='btn_search' onclick='reg_click();'>배달대행사 등록</button>
		...
		<div class="hide_div">
			<form id="channel_reg">
				<p id="reg_title">배달대행사 등록</p>
				<div class="reg_box">
					<table>
						<tr>
							<th class='mtitle'>배달대행사코드<p id="emp">*</p></th>
							<td><input type="text" class="reg_input" id="CPN_CD" name="CPN_CD" autocomplete='off'/></td>
						</tr>
						<tr>
							<th class='mtitle'>배달대행사명<p id="emp">*</p></th>
						...
						<tr>
							...
							<td><input type="text" class="reg_input" id="CPN_TEL" onKeyup="inputTelNumber(this);" name="CPN_TEL" autocomplete='off'/></td>
						</tr>
						<tr>
							<th class='mtitle'>담당자명</th>
				...
				<div class="upload_div2">    
					...
					<button type="reset" class='btn_search' onclick='reg_cancel();'>취소</button>
				</div>
			</form>
		</div>
	</div>
